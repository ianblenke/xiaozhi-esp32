...
The MCP protocol in this project is used for communication between the backend API (MCP client) and ESP32 devices (MCP server), so that the backend can discover and invoke functions (tools) provided by the device.
...
- `result`: Result of successful method execution (for Success Response).
...
1.  **Connection Establishment and Capability Announcement**
    ...
    - **Sender:** Device.
    - **Message:** The device sends a base protocol "hello" message to the backend API, which includes the list of capabilities supported by the device, for example, MCP protocol support (`"mcp": true`).
...
2.  **Initialize MCP Session**
    ...
    - **Timing:** After the backend API receives the device "hello" message and confirms the device supports MCP, typically sent as the first request of the MCP session.
    ...
    - **Device response message (MCP payload):**
      ```json
      {
        ...
        "result": {
          ...
          "capabilities": {
            "tools": {} // The tools here don't list detailed info, requires tools/list
          },
          "serverInfo": {
    ...
    - **Device response message (MCP payload):**
      ```json
      {
        ...
        "result": {
          ...
          "nextCursor": "..." // If the list is large and needs pagination, this contains the cursor value for the next request
        }
      }
      ```
...
5.  **Device-Initiated Messages (Notifications)**
    - **Timing:** When an internal device event occurs that needs to notify the backend API (e.g., state changes. Although the code examples don't show explicit tools sending such messages, the existence of `Application::SendMcpMessage` implies the device may proactively send MCP messages).
...
This document outlines the main interaction flow of the MCP protocol in this project. For specific parameter details and tool functions, please refer to `McpServer::AddCommonTools` in `main/mcp_server.cc` and the implementation of each tool.
