...
This guide introduces how to create a new board initialization program for the XiaoZhi AI voice chatbot project. XiaoZhi AI supports over 70 ESP32 series development boards, with each board's initialization code placed in its corresponding directory.

## Important Notice

> **Warning**: For custom boards, when the IO configuration differs from the original board, do not directly overwrite the original board's configuration to compile firmware. You must create a new board type, or use different name and sdkconfig macro definitions in the builds configuration of the config.json file to differentiate. Use `python scripts/release.py [board_directory_name]` to compile and package the firmware.
>
> If you directly overwrite the original configuration, during future OTA upgrades, your custom firmware may be overwritten by the standard firmware of the original board, causing your device to malfunction. Each board has a unique identifier and a corresponding firmware upgrade channel, so maintaining the uniqueness of the board identifier is very important.
...
- `config.json` - Build configuration, specifying the target chip and special build options
...
First, create a new directory under the `boards/` directory, using the naming convention `[brand-name]-[board-type]`, for example `m5stack-tab5`:
...
Define the build configuration in `config.json`. This file is used by the `scripts/release.py` script for automated building:
...
```json
{
    ...
    "builds": [
        {
            "name": "my-custom-board",  // Board name, used for generating firmware packages
            "sdkconfig_append": [
...
- `name`: Name of the compiled firmware package, recommended to match the directory name
- `sdkconfig_append`: Array of additional sdkconfig configuration items, appended to the default configuration
...
**Notes:**
- `BOARD_TYPE_MY_CUSTOM_BOARD` is the configuration item name, which should be all uppercase with underscore separators
...
Open the `main/CMakeLists.txt` file and add a new configuration in the board type conditional section:
...
Choose the appropriate font size based on screen resolution:
...
If your board directory contains a `config.json` file, you can use this script to automatically complete configuration and compilation:
...
- Apply the build options from `sdkconfig_append`
- Complete compilation and package the firmware

### 6. Create README.md

In README.md, describe the board's features, hardware requirements, and build and flash steps.
...
Various MCP tools can be added to enable AI capabilities:
...
1. **Reference similar boards**: If your new board is similar to an existing board, you can refer to the existing implementation
2. **Step-by-step debugging**: Implement basic functionality first (such as display), then add more complex features (such as audio)
